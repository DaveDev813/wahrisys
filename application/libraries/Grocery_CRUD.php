...
class grocery_CRUD_Field_Types {
    ...
    public function __construct() {
        $ci = &get_instance(); // rodave 
        $ci->load->helper('debugging');
    }
    ...
    public function get_field_types() {
        ...
        foreach ($this->basic_model->get_field_types_basic_table() as $field_info) {
            ...
            if( isset($this->display_as[$field_info->name]) ) {
                
                $field_info->display_as = $this->display_as[$field_info->name];
            }
            else {
                
                $field_info->display_as = ucfirst(str_replace("_", " ", $field_info->name));
            }
        ...
        if ( !empty($db_type->type) ) {
            ...
            switch ( $db_type->type ) {
                ...
                case 'longint':
                    ...
                    $type = ($db_type->db_type == 'tinyint' && $db_type->db_max_length == 1) ? 'true_false' : 'integer';
                ...
                case 'enum':
                    ...
                    $type = ($db_type->db_type != 'enum') ? 'string' : 'enum';
                    break;
                
                case 'set':
                    
                    $type = ($db_type->db_type != 'set') ? 'string' : 'set';
                    break;
    ...
    protected function db_insert_validation() {
        ...
        if (!empty($this->validation_rules)) {
            ...
            if ($form_validation->run()) {
                ...
                if (empty($validation_result->error_message) and empty($validation_result->error_fields)) { // rodave_fix
                    $validation_result->success = true;
                }
            }
        } else {
...
class grocery_CRUD_Layout extends grocery_CRUD_Model_Driver {
    ...
    protected function get_add_input_fields($field_values = null) {
        ...
        foreach ($fields as $field_num => $field) {
            ...
            if( !empty($field_values) && isset($field_values->{$field->field_name})  ){
...
//                    if (!isset($field->db_extra) || $field->db_extra != 'auto_increment') { // rodave
                        if (isset($this->display_as[$field->name])) {
